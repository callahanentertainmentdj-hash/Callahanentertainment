...
def _verify_state(state: str):
    ...
    if abs(time.time() - stamp) > 900:
        raise HTTPException(
            status_code=400,
            detail="OAuth state expired; start authorization again",
        )
...
async def _exchange_refresh_token() -> str:
    ...
    if (
        ...
        and now < _access_token_cache["expires"] - 60
    ):
    ...
    if response.status_code >= 400:
        ...
        raise HTTPException(
            status_code=502,
            detail=(
                f"Google token refresh failed (HTTP {response.status_code})"
    ...
    _access_token_cache["expires"] = (
        now + int(payload.get("expires_in", 3600))
    )
...
def _ga_channel_rows(payload):
    ...
    metric_names = [
        h.get("name")
        for h in payload.get("metricHeaders", [])
    ]
...
async def _ga_report(
    ...
    return await _google_request(
        ...
        (
            ...
            f"v1beta/properties/{prop}:runReport"
        ),
        json=payload,
    )
...
async def _ads_search(
    ...
    cid = (
        customer_id or GOOGLE_ADS_CUSTOMER_ID
    ).replace("-", "").strip()

    if not cid:
        raise HTTPException(
            status_code=503,
            detail="GOOGLE_ADS_CUSTOMER_ID is not configured",
        )
...
async def google_oauth_callback(
    ...
    if response.status_code >= 400:
        return HTMLResponse(
            (
                ...
                f"<p>HTTP {response.status_code}</p>"
            ),
            status_code=502,
        )
    ...
    if not refresh_token:
        return HTMLResponse(
            "<h2>Authorization succeeded, but Google did not "
            "return a refresh token.</h2>"
            "<p>Remove the app from Google permissions and run "
            "/google/oauth/start again.</p>"
        )
...
async def google_status(
    ...
    result = {
        ...
        "searchConsoleSiteConfigured": bool(
            SEARCH_CONSOLE_SITE_URL
        ),
        "analyticsPropertyConfigured": bool(
            GA4_PROPERTY_ID
        ),
        "adsCustomerConfigured": bool(
            GOOGLE_ADS_CUSTOMER_ID
        ),
        "adsDeveloperTokenConfigured": bool(
            GOOGLE_ADS_DEVELOPER_TOKEN
        ),
        "adsApiVersion": GOOGLE_ADS_API_VERSION,
        "businessAccountConfigured": bool(
            GOOGLE_BUSINESS_ACCOUNT_ID
        ),
        "businessLocationConfigured": bool(
            GOOGLE_BUSINESS_LOCATION_ID
        ),
    }
...
async def search_console_performance(
    ...
    site = (
        site_url or SEARCH_CONSOLE_SITE_URL
    ).strip()
...
async def search_console_opportunities(
    ...
    min_position: float = Query(
        default=3.0,
        ge=1.0,
        le=100.0,
    ),
    max_position: float = Query(
        default=15.0,
        ge=1.0,
        le=100.0,
    ),
    ...
    if max_position < min_position:
        raise HTTPException(
            status_code=400,
            detail=(
                "max_position must be >= min_position"
            ),
        )

    start, end = _default_dates(days)

    queries = await _sc_report(
        site,
        start,
        end,
        ["query"],
...
async def analytics_report(
    ...
    dimensions: str = Query(
        ...
        description=(
            "Comma-separated GA4 dimensions"
        ),
    ),
    metrics: str = Query(
        default=(
    ...
    prop = (
        property_id or GA4_PROPERTY_ID
    ).replace(
        "properties/",
        "",
    ).strip()
...
# -------------------------------------------------------------------
# GOOGLE REVIEWS - REPLY / UPDATE OWNER REPLY
# -------------------------------------------------------------------
...
class ReviewReplyRequest(BaseModel):
    comment: str = Field(
        ...
        max_length=4096,
        description=(
            "Reply text to publish publicly "
            "on the Google review"
        ),
    )
...
async def marketing_compare(
    ...
    current_start, current_end = (
        _period_dates(days, 0)
    )

    previous_start, previous_end = (
        _period_dates(days, 1)
    )

    sc_current = _sc_totals(
        await _sc_report(
            site,
            current_start,
            current_end,
...
async def marketing_overview(
    ...
    sc_total_payload = await _sc_report(
        site,
        start,
        end,
        None,
    ...
    sc_queries_payload = await _sc_report(
        site,
        start,
        end,
        ["query"],
    ...
    sc_pages_payload = await _sc_report(
        site,
        start,
        end,
        ["page"],
    ...
    sc_totals = _sc_totals(
        sc_total_payload
    )

    ga_totals = _ga_metric_map(
        ga_totals_payload
    )

    channels = _ga_channel_rows(
        ga_channels_payload
    )
    ...
    return {
        ...
        "notes": [
            (
                "Search Console clicks and GA4 "
                "Organic Search sessions are different "
                "measurements and should not be expected "
                "to match exactly."
            ),
            (
                "Booking attribution is not claimed here "
                "because the current InflatableOffice "
                "bridge does not yet prove a reliable "
                "source/UTM relationship for each booking."
            ),
        ],
    }
...
async def marketing_summary(
    ...
    if (
        GOOGLE_BUSINESS_ACCOUNT_ID
        and GOOGLE_BUSINESS_LOCATION_ID
    ):
        aid = (
            GOOGLE_BUSINESS_ACCOUNT_ID
            .replace(
