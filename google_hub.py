...
async def _exchange_refresh_token() -> str:
    ...
    if not GOOGLE_REFRESH_TOKEN:
        raise HTTPException(
            status_code=503,
            detail=(
                "Google OAuth is not fully connected. "
                "Visit /google/oauth/start, authorize Google, "
                "then save GOOGLE_REFRESH_TOKEN in Render."
            ),
        )
...
async def google_oauth_callback(
    ...
    if not refresh_token:
        return HTMLResponse(
            "<h2>Authorization succeeded, but Google "
            "did not return a refresh token.</h2>"
            "<p>Remove the app from your Google account "
            "permissions and run /google/oauth/start again."
            "</p>"
        )
    ...
    return HTMLResponse(
        ...
        "<p>Copy the token below into Render as "
        "<b>GOOGLE_REFRESH_TOKEN</b>. "
        "Treat it like a password.</p>"
        f"<textarea style='width:95%;height:120px'>"
        f"{safe_token}</textarea>"
        "<p>Save it in Render, redeploy, then test "
        "/google/status.</p>"
    )
    ...
    if not site:
        raise HTTPException(
            ...
            detail=(
                ...
                "/google/search-console/sites first"
            ),
        )
...
async def search_console_summary(
    ...
    return {
        ...
        "topQueries": await report(
            ["query"],
            50,
        ),
        "topPages": await report(
            ["page"],
            50,
        ),
        "devices": await report(
            ["device"],
            10,
        ),
        "countries": await report(
            ["country"],
            25,
        ),
        "daily": await report(
